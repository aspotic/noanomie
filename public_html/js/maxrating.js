function showIt(item,i,j,max, imgDir){
   var id;
   actualItem = item;
   imgDirectory = imgDir;
   ...
   for (var x=1;x<=i;x++){
      id = item + "_" + x;
      document.getElementById(id).src = imgDir + "b.gif";
   }
   
   // Display the not selected ones
   for (var x=i+1;x<=max;x++){
      id = item + "_" + x;
      if (x<=j) document.getElementById(id).src = imgDir + "y.gif";
      else document.getElementById(id).src = imgDir + "w.gif";
   }
}

function showOriginal(i,max, imgDir){
   for (var x=1;x<=max;x++){
      id = actualItem + "_" + x;
      if (x<=i) document.getElementById(id).src = imgDir + "y.gif";
      else document.getElementById(id).src = imgDir + "w.gif";
   }
}
...
function removeActions(rating,max){
      showOriginal(rating,max,imgDirectory);
      var t;
      for (var x=1;x<=max;x++){
         id = actualItem + "_" + x;
...
function submitRating(item,rate,max, directory){
   httpObject = getHTTPObject();
   actualItem = item;
   if (httpObject != null) {
      httpObject.open("GET", directory + "functions.php?item="+item+"&rating="+rate+"&max="+max, true);
